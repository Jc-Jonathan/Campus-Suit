...
export const Notificationsc: React.FC = () => {
  ...
  const sendNotification = async () => {
    ...
    try {
      ...
      const notificationPromises = selectedApps.map(async (app) => {
        const notificationData = {
          message: message.trim(),
          category: 'SCHOLARSHIP',
          targetType: 'USERS',
          targetEmails: [app.email], // Use targetEmails array for consistency
          scholarshipInfo: {
            applicantName: app.fullName,
            applicantEmail: app.email,
            scholarshipName: app.scholarshipTitle || app.scholarshipName || 'Scholarship Program',
            courseName: app.educationalLevel || 'Program',
            message: 'admin notification'
          }
        };

        return await axios.post('https://pandora-cerebrational-nonoccidentally.ngrok-free.dev/api/notifications/scholarship', notificationData, {
          headers: {
      ...
      Alert.alert('Success', `Notification sent to ${selectedApps.length} student(s)`);
      setMessage('');
