...
export default function NotificationScreen() {
  ...
  const renderFormattedMessage = (message: string, category: string, item: any) => {
    ...
    if (category === 'SCHOLARSHIP' && item.scholarshipInfo) {
      // Check if this is an application submission notification
      if (item.scholarshipInfo.message === 'application submitted') {
        return (
          <Text style={[styles.message, messageStyle]}>
            Hello {item.scholarshipInfo.applicantName},
            {'\n\n'}
            Congratulations on your application for "{item.scholarshipInfo.scholarshipName}" under "{item.scholarshipInfo.courseName}".
            {'\n\n'}
            Always check your email for daily updates.
          </Text>
        );
      }
      
      // Check if this is an admin notification
      if (item.scholarshipInfo.message === 'admin notification') {
        return (
          <Text style={[styles.message, messageStyle]}>
            {item.message}
          </Text>
      ...
      // This is a status update notification (approved/rejected)
      return (
        <Text style={[styles.message, messageStyle]}>
          Hello {item.scholarshipInfo.applicantName},
          {'\n\n'}
          Your application for "{item.scholarshipInfo.scholarshipName}" under "{item.scholarshipInfo.courseName}" has been {item.scholarshipInfo.message?.includes('approved') ? 'APPROVED' : 'REJECTED'}.
        </Text>
      );
    }
    ...
    if (category === 'SHOP' && item.orderDetails) {
            ...
            <View style={styles.orderDetailsContainer}>
              <View style={styles.orderHeader}>
                ...
                <View style={[
                  ...
                  <Text style={styles.statusText}>{status?.toUpperCase() || 'PENDING'}</Text>
                </View>
              </View>
              
              {/* Show all ordered products */}
              <View style={styles.productsContainer}>
                <Text style={styles.productsTitle}>Ordered Items ({itemCount}):</Text>
                {(item.orderDetails?.items || []).map((product: any, index: number) => (
  ...
  const shouldShowNotification = (notification: Notification, userEmail?: string) => {
    ...
    // For scholarship notifications, check if notification belongs to current user
    if (notification.category === 'SCHOLARSHIP') {
      // Check if current user's email is in targetEmails array
      if (notification.targetEmails && notification.targetEmails.length > 0 && userEmail) {
        return notification.targetEmails.includes(userEmail);
      }
      // Check if notification has scholarshipInfo with applicantEmail that matches current user
    ...
    if (notification.category === 'SHOP') {
      ...
      // Check if notification has orderDetails with customerEmail that matches current user
      if (notification.orderDetails?.customerEmail) {
        return notification.orderDetails.customerEmail === userEmail;
      }
      // Fallback to old logic for backward compatibility
      if (notification.targetUser) {
        return notification.targetUser === userEmail;
          ...
          return (
            <TouchableOpacity
              ...
              onPress={async () => {
                ...
                if ((item.category === 'SHOP' && (item.orderDetails || item.orderDetails)) || (item.category === 'LOAN' && item.loanInfo)) {
