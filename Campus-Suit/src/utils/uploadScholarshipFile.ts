...
export interface UploadedScholarshipFile {
  uri: string;
  name: string;
  type: string | null;
  size: number;
...
export const uploadScholarshipFileToCloudinary = async (file: UploadedScholarshipFile): Promise<UploadedScholarshipFile> => {
  try {
    console.log(`📤 Uploading scholarship course file: ${file.name} to Cloudinary...`);
    ...
    console.log(`✅ Scholarship file uploaded successfully: ${file.name}`);
