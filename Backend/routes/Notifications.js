    ...
    if (category === 'SCHOLARSHIP') {
      ...
      query = { 
        ...
        $or: [
          ...
          { reader: userEmail },
          { targetEmails: { $in: [userEmail] } }  // Check if user email is in targetEmails array
        ]
      };
      ...
      query = { 
        ...
        $or: [
          { targetType: 'ALL' },
          { 'orderDetails.customerEmail': userEmail },
          { targetUser: userEmail },
          { targetEmails: userEmail }  // Use targetEmails instead of targetUsers
        ]
      };
...
router.post('/scholarship', auth, async (req, res) => {
  try {
    ...
    // Extract email from scholarshipInfo or targetEmails or reader for user-specific targeting
    const targetEmail = scholarshipInfo?.applicantEmail || (targetEmails && targetEmails[0]) || reader;
    
    const notification = await Notification.create({
      message,
      category, // 'SCHOLARSHIP'
      targetType: targetType || 'ALL',
      targetEmails: targetEmails || [], // Use targetEmails array for consistency
      scholarshipInfo,
      // Add reader field for backward compatibility
...
router.post('/shop', auth, async (req, res) => {
  try {
    ...
    console.log('Received shop notification data:', {
      ...
      targetUser,
      orderDetails,
      orderItemsCount: orderDetails?.items?.length || 0,
      totalPrice: orderDetails?.totalAmount
    });

    // Extract email from orderDetails for user-specific targeting
    const targetEmail = orderDetails?.customerEmail || targetUser;
