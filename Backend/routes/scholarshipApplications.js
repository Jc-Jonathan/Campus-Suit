...
Best regards,
Leleaninternational Scholarship Program foundation
`;
    }
...
router.post('/', upload.none(), async (req, res) => {
  try {
    ...
    // CREATE SCHOLARSHIP APPLICATION NOTIFICATION
    try {
      const notificationMessage = `Hello ${req.body.fullName},\n\nCongratulations on your application for "${req.body.scholarshipTitle}" under "${req.body.scholarshipName}".\n\nAlways check your email for daily updates.`;

      await Notification.create({
        message: notificationMessage,
        category: 'SCHOLARSHIP',
        targetType: 'USERS',
        targetEmails: [req.body.email], // Target only the applicant's email
        scholarshipInfo: {
          applicantName: req.body.fullName,
          applicantEmail: req.body.email,
          scholarshipName: req.body.scholarshipTitle,
          courseName: req.body.scholarshipName,
          message: 'application submitted'
        }
      });
      
      console.log('✅ Scholarship application notification created for:', req.body.email);
    } catch (notificationError) {
      console.error('❌ Failed to create scholarship application notification:', notificationError);
